...
///
shelf.Handler initHandler() {
  final staticHandler = createStaticHandler(rootPath(DEFAULT_DIR),
      defaultDocument: DEFAULT_INDEX);

  final handlerCascade =
      new shelf.Cascade().add(staticHandler).add(_browseRequest).handler;

  return new shelf.Pipeline()
...
Future<shelf.Response> _browseRequest(shelf.Request request) async {
  ...
  if (await FileSystemEntity.isDirectory(rootDir + request.url.toFilePath())) {
    ...
    resp = await html(await getDirContentUl(rootDir, requestUrl, resp),
        showBack: !isHome);
  } else if (extension(request.url.toFilePath()) == ".md") {
    String mdContent =
        await new File(rootDir + request.url.toFilePath()).readAsString();
